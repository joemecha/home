...
class BuildingTest < Minitest::Test
  ...
  def test_rented_units_if_renter
    building = Building.new
    unit1 = Apartment.new({number: "A1", monthly_rent: 1200, bathrooms: 1, bedrooms: 1})
    unit2 = Apartment.new({number: "B2", monthly_rent: 999, bathrooms: 1, bedrooms: 2})
    unit3 = Apartment.new({number: "C3", monthly_rent: 1150, bathrooms: 2, bedrooms: 2})
  ...
  def test_units_by_number_of_bedrooms
    #returns arrays of unit numbers, grouped
  end

  def test_annual_breakdown
    #returns hash of renter keys and annual rent values
  end

  def rooms_by_renter
    #returns hash of renter keys and value: sub hash of room type keys and quantity integer
  end
end
